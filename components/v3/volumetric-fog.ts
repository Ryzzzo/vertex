/**
 * Raymarched volumetric fog. Ported from `components/v2/volumetric-fog.ts`.
 *
 * ── What changed for v3, and what deliberately did not ────────────────────
 *
 * The march itself is unchanged, because everything the v2 log records about
 * it (§5) is still true and was expensive to learn: the frequency constant, the
 * HG normalisation against real source intensities, the useful density range
 * being only ~1.5× wide, the self-shadowing proxy's sign. Re-deriving any of
 * that would be paying twice.
 *
 * What is new is that the fog now has to be *two* fogs. The ship is a cool
 * blue-lit interior bank sitting on a deck; the Dune is a warm ochre haze over
 * an open plain lit by one low sun. Rather than fork the shader, the light
 * positions, colours, key direction and floor plane are all settable per frame,
 * and `SceneGL` writes them from the blended palette. One shader, one pass, and
 * the act transition is a continuous interpolation of six uniforms rather than
 * a swap between two effects — which is what lets the palette cross the act
 * break smoothly instead of cutting with the geometry.
 *
 * The original header follows, because its reasoning is the reason this file
...
export class VolumetricFogPass extends Pass {
  ...
  constructor(camera: THREE.PerspectiveCamera) {
    ...
    this.marchMat = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        uFar: { value: 44 },
        /**
         * Three practicals. In the ship these are the two ceiling LED runs and
         * the floor wash; in the Dune they collapse toward the horizon under
         * the sun. `SceneGL` writes them every frame from the blended palette,
         * so these are only the first frame's values.
         */
        uLightPos: {
          value: [
            new THREE.Vector3(-2.6, 4.3, -3.0),
            new THREE.Vector3(2.6, 4.3, -3.0),
        ...
        /**
         * The plane density falls off above. The bridge deck is y = 0; the Dune
         * plain is also y = 0, which is a happy accident of sharing an origin
         * and means this only moves during the launch, when the camera is above
         * the deck and the bank should be below it.
         */
        uFloorY: { value: 0.02 },
      },
    });
  ...
  /* ── The v3 additions: everything the two acts need to differ in ────────
     Written per frame by SceneGL from the blended palette. Scalars rather
     than a palette object on purpose — this class should not know that acts
     exist, only that its lights can move. */

  /** Moves one of the three practicals. `i` is 0-2. */
  setLightPos(i: number, x: number, y: number, z: number) {
    (this.marchMat.uniforms.uLightPos.value as THREE.Vector3[])[i].set(x, y, z);
  }

  /**
   * Sets one practical's colour and intensity.
   *
   * `rgb` is expected already desaturated toward grey — v2 §5 records that
   * lighting vapour with an accent at full chroma produces a solid coloured
   * bank rather than fog. `palette.ts` does that halving, so the caller passes
   * `vapour` and not `key`.
   */
  setLightCol(i: number, rgb: readonly [number, number, number], intensity: number) {
    (this.marchMat.uniforms.uLightCol.value as THREE.Color[])[i].setRGB(
      rgb[0] * intensity,
      rgb[1] * intensity,
      rgb[2] * intensity,
    );
  ...
  /** The key that the phase function reads — the sun in the Dune act. */
  setKey(dir: THREE.Vector3, rgb: readonly [number, number, number], intensity: number) {
    (this.marchMat.uniforms.uKeyDir.value as THREE.Vector3).copy(dir).normalize();
    (this.marchMat.uniforms.uKeyCol.value as THREE.Color).setRGB(
      rgb[0] * intensity,
      rgb[1] * intensity,
      rgb[2] * intensity,
    );
  ...
  /** The plane the bank sits on. */
  setFloor(y: number) {
    this.marchMat.uniforms.uFloorY.value = y;
  }
